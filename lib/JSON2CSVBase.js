...
const validDecimalFormat = [currencyFormat, priceFormat];
class JSON2CSVBase {
  constructor(opts) {
    ...
    return fields.map((fieldInfo) => {
      ...
      if (typeof fieldInfo === 'object') {
        ...
        if (typeof fieldInfo.value === 'string') {
          let decimalFormat = (fieldInfo.decimalFormat && validDecimalFormat.includes(fieldInfo.decimalFormat)) && fieldInfo.decimalFormat;
          return {
  ...
  getHeader() {
    return fastJoin(
      this.opts.fields.map(fieldInfo => this.processValue(fieldInfo.label, { stringify: true })),
      this.opts.delimiter
    );
  }
   ...
   */
  processValue(value, fieldInfo) {
    let { stringify, decimalFormat } = fieldInfo
    if (value === null || value === undefined) {
      return undefined;
    }

    const valueType = typeof value;
    if (valueType === 'number' && decimalFormat) {
      if (decimalFormat === priceFormat) {
        value = parseFloat(value).toFixed(2);
      }
      if (decimalFormat === currencyFormat) {
